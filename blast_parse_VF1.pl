...
open(OUTFILE, ">E_1_in_a_mill_CD_strain_AAAAA_ALL_PROT_BLAST_PARSE1_VF.txt");
...
my $response = $browser->request(POST ($url, [DATALIB => "VFs.faa", HITLIST_SIZE => "5",	
FILTER => "L", EXPECT => "0.0000001", PROGRAM  => "blastp", SEQUENCE => $arrayOfSequences[$i], CMD => "Put"])); 	
...
print "Open E_1_in_a_mill_CD_strain_AAAAA_ALL_PROT_BLAST_PARSE1_VF.txt to view the results.\n";
