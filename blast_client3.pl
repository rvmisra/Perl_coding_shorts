...
while (my $sequenceEntry = <SEQFILE>) {
    ...
    my $response = $browser->request(POST ($BLAST_URL, [DATABASE => $settings{DATABASE}, HITLIST_SIZE => $settings{HITLIST_SIZE}, EXPECT => $settings{EXPECT}, WORD_SIZE => $settings{WORD_SIZE}, FILTER => $settings{FILTER}, PROGRAM => $settings{PROGRAM}, MATRIX => $settings{MATRIX}, EXISTENCE => $settings{EXISTENCE},  EXTENSION => $settings{EXTENSION}, QUERY => $query, CMD => "Put"]));
...
sub _setDefaults {
	...
	$settingsHash->{MATRIX} = "PAM30";
	$settingsHash->{NUM_SEQ} = "1";
	$settingsHash->{EXISTENCE} = "10";
	$settingsHash->{EXTENSION} = "1";
