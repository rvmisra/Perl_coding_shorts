...
($query, $subject, $percent_id, $alignment_length, $mismatches, $gap_openings, $q_start, $q_end, $e_value, $bit_score) = split /\t/, $line;


if ($percent_id =~ /100/) {

print OUTFILE "$query\t$subject\t$match_desc\t$percent_id\t$alignment_length\t$mismatches\t$gap_openings\t$q_start\t$q_end\t$e_value\t$bit_score\n";
#print OUTFILE "$query\n";
