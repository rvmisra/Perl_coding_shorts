...
open(OUTFILE, ">NCBI_blast_result_PEP_search1.txt") or die ("Cannot open file : $!");
...
for (my $i = 0; $i < scalar(@arrayOfRID); $i = $i + 1) {
	...
	if ($result !~ m/Bacillus/) {
			  print(OUTFILE "Results for XXXXXXX : $arrayOfNames[$i] : XXXXXXX.\n");
		      $resultFound = 1;
	}
...
print "Open NCBI_blast_result_PEP_search1.txt to view the results.\n";
