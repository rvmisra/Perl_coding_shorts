...
#my $fileToRead = <STDIN>;
#chomp($fileToRead);
open (PROTEINFILE, "<Prokka_faa_vs_COGcodeseq_maxtargetseqs1_Evalue_e10_OUT.txt") or die( "Cannot open file : $!" );
open(OUTFILE, ">COGcount_OUT2.txt");
...
my $pcj = ($count_j/$totalincdifftoadd)*100;
my $pca = ($count_a/$totalincdifftoadd)*100;
my $pck = ($count_k/$totalincdifftoadd)*100;
my $pcl = ($count_l/$totalincdifftoadd)*100;
my $pcb = ($count_b/$totalincdifftoadd)*100;
my $pcd = ($count_d/$totalincdifftoadd)*100;
my $pcy = ($count_y/$totalincdifftoadd)*100;
my $pcv = ($count_v/$totalincdifftoadd)*100;
my $pct = ($count_t/$totalincdifftoadd)*100;
my $pcm = ($count_m/$totalincdifftoadd)*100;
my $pcn = ($count_n/$totalincdifftoadd)*100;
my $pcz = ($count_z/$totalincdifftoadd)*100;
my $pcw = ($count_w/$totalincdifftoadd)*100;
my $pcu = ($count_u/$totalincdifftoadd)*100;
my $pco = ($count_o/$totalincdifftoadd)*100;
my $pcc = ($count_c/$totalincdifftoadd)*100;
my $pcg = ($count_g/$totalincdifftoadd)*100;
my $pce = ($count_e/$totalincdifftoadd)*100;
my $pcf = ($count_f/$totalincdifftoadd)*100;
my $pch = ($count_h/$totalincdifftoadd)*100;
my $pci = ($count_i/$totalincdifftoadd)*100;
my $pcp = ($count_p/$totalincdifftoadd)*100;
my $pcq = ($count_q/$totalincdifftoadd)*100;
my $pcr = ($count_r/$totalincdifftoadd)*100;
my $pcs = ($count_s/$totalincdifftoadd)*100;

my $totalpcfunknown = 100 - ($pcj + $pca + $pck + $pcl + $pcb + $pcy + $pcv + $pct + $pcm + $pcn + $pcz + $pcw +
 $pcu + $pco + $pcc + $pcg + $pce + $pcf + $pch + $pci + $pcp + $pcq);

print $totalpcfunknown;
...
print OUTFILE "[RS] Poorly/Uncharacterised:" . "\t" . "\t" . $totalpcfunknown . "\n";
