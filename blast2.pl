...
for (my $i = 0; $i < scalar(@arrayOfSequences); $i = $i + 1) {
    ...
    my $response = $browser->request(POST ($url, [DATABASE => "nr", HITLIST_SIZE => "20",
...
open(OUTFILE, ">NCBI_blast_results1.txt") or die ("Cannot open file : $!");
...
for (my $i = 0; $i < scalar(@arrayOfRID); $i = $i + 1) {
	...
	while ($resultFound == 0) {
		...
		if ($result =~ m/\s\S(Ac)\w\s/){
      		  print(OUTFILE "Results for : $arrayOfNames[$i] : . **** NOT BACILLI *****\n");
			  $resultFound = 1;
	}

	elsif ($result =~ m/\s\S (Bac)\w/) {
			  print(OUTFILE "Results for : $arrayOfNames[$i] : . ----- IS BACILLI --------\n");
		      $resultFound = 1;
	}		
		elsif ($result =~ m/\s\S[C-Z]\w\s/){
      		  print(OUTFILE "Results for : $arrayOfNames[$i] : . ##### NOT BACILLI #####\n");		  
		      $resultFound = 1;
	}
...
print "Open NCBI_blast_results1.txt to view the results.\n";
