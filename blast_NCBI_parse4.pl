...
for (my $i = 0; $i < scalar(@arrayOfSequences); $i = $i + 1) {
    ...
    my $response = $browser->request(POST ($url, [DATABASE => "nr", HITLIST_SIZE => "10",
    FILTER => "L", PROGRAM  => "blastp", EXPECT => "1.0", QUERY => $arrayOfSequences[$i], CMD => "Put"]));
...
open(OUTFILE, ">NCBI_blast_results_UNIQUE_CUPID_test1.txt") or die ("Cannot open file : $!");
...
for (my $i = 0; $i < scalar(@arrayOfRID); $i = $i + 1) {
	...
	while ($resultFound == 0) {
		...
		my $result = $response->as_string();
		    ...
		    print "Writing the results to NCBI_blast_results_UNIQUE_CUPID6.txt\n"; 
...
#matches.    		
		    if ($result =~ m/(?!\W)...bacill/) {      
   			   print (OUTFILE "Results for $arrayOfNames[$i] : ");
    		   print (OUTFILE "######## No good, is not unique :\n\n");
	...
	elsif ($result =~ m/Sacch/) {
			  print(OUTFILE "Results for : $arrayOfNames[$i] : ");
	          print (OUTFILE "&&&&&&& No good, is not unique :\n\n");
			  	      $resultFound = 1;
	}
	
	elsif ($result =~ m/Clost/) {
			  print(OUTFILE "Results for : $arrayOfNames[$i] : ");
	          print (OUTFILE "%%%%%%% No good, is not unique :\n\n");
			  	      $resultFound = 1;
	}
	
	elsif ($result =~ m/Tetrahymena/) {
			  print(OUTFILE "Results for : $arrayOfNames[$i] : ");
	          print (OUTFILE "$$$$$$$$$ No good, is not unique :\n\n");
			  	      $resultFound = 1;
	}
...
print "Open NCBI_blast_results_UNIQUE_CUPID_test1.txt to view the results.\n";
