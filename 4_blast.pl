...
for (my $i = 0; $i < scalar(@arrayOfSequences); $i = $i + 1) {
    ...
    FILTER => "L", PROGRAM  => "blastp", QUERY => $arrayOfSequences[$i], CMD => "Put"]));
...
open(OUTFILE, ">blast_results.txt") or die ("Cannot open file : $!");
...
print "Open blast_results.txt to view the results.\n";
