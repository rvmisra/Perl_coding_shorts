...
my $response = $browser->request(POST ($url, [DATALIB => "TVFacs", HITLIST_SIZE => "1",	
FILTER => "L", EXPECT => "1e-10", PROGRAM  => "blastp", SEQUENCE => $arrayOfSequences[$i], CMD => "Put"])); 	
...
print "Open CD_TEST_ALL_PROT_BLAST_PARSE1_TVFAC.txt to view the results.\n";
