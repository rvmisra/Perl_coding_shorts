...
for (my $i = 0; $i < scalar(@arrayOfSequences); $i = $i + 1) {
    ...
    my $response = $browser->request(POST ($url, [DATABASE => "nr", HITLIST_SIZE => "10",
    FILTER => "L", PROGRAM  => "blastp", EXPECT => "0.0001", QUERY => $arrayOfSequences[$i], CMD => "Put"]));
...
open(OUTFILE, ">NCBI_blast_results_UNIQUE_CUPID5.txt") or die ("Cannot open file : $!");
...
for (my $i = 0; $i < scalar(@arrayOfRID); $i = $i + 1) {
	...
	while ($resultFound == 0) {
		...
		my $result = $response->as_string();
		    ...
		    print "Writing the results to NCBI_blast_results_UNIQUE_CUPID5.txt\n"; 
...
print "Open NCBI_blast_results_UNIQUE_CUPID5.txt to view the results.\n";
