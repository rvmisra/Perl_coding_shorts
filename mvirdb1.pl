...
my $url = "http://greengenes.llnl.gov/kpath/bin/virulence/virulenceBlast.pl";
...
open(OUTFILE, ">BAA_ALL_PROT_BLAST_PARSE1_MVIRDB.txt");
...
#my $DB = "/dna2/www/kpath/lib/Virulence/data/virulenceDB.protein.fasta";

#We have our virtual browser that we want to use to submit BLAST queries.                 	
my $response = $browser->request(POST ($url, [filer => "0", SEQUENCE => $arrayOfSequences[$i], CMD => "Put"])); 	
...
print "Open BAA_ALL_PROT_BLAST_PARSE1_MVIRDB.txt to view the results.\n";